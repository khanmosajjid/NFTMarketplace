...
$("#set-Royalty-percentage").change(function () {
...
function createNFT() {
    ...
    let sSetRRoyaltyPercentage = ($("#set-Royalty-percentage").is(":checked")) ? $("#royalty-percentage").val() : 0;
    let sNftdescription = $('#nftdescription').val();
    let eAuctionType = $('input[name="auction-type"]:checked').val();
    let nBasePrice = $('#basePrice').val();

    if (isNaN(sSetRRoyaltyPercentage) || sSetRRoyaltyPercentage > 100) {
    ...
    function create() {
        ...
        fd.append('sSetRoyaltyPercentage', sSetRRoyaltyPercentage);
        ...
        try {
            ...
            if (ee != undefined) {
                ...
                if (web3 && network == 97 && ee != undefined) {
                    ...
                    let nEstimatedGasLimit = await oContract.methods.mintToken(parseInt(fd.nQuantity) > 1 ? true : false, fd.sHash, fd.sName, parseInt(fd.nQuantity), fd.sSetRRoyaltyPercentage, fd.sCollaborator, fd.nCollaboratorPercentage).estimateGas({
                    ...
                    oContract.methods.mintToken(parseInt(fd.nQuantity) > 1 ? true : false, fd.sHash, fd.sName, parseInt(fd.nQuantity), fd.sSetRRoyaltyPercentage, fd.sCollaborator, fd.nCollaboratorPercentage)
                        .send({
                            from: default_Address,
