...
$.ajax({
    ...
    success: (result, status, xhr) => {
        ...
        $("#nBasePrice,#modal-nBasePrice").text(result.data.nBasePrice.$numberDecimal + " MATIC");
        ...
        if (uID != result.data.oCurrentOwner._id) {
            ...
            $("#btnUpdatePrice").on("click", function () {
                ...
                if ($('#txtNewPrice').val().trim() == "") {
                    ...
                    $("#modal-bid-final-amount").text(`${(+$('#bidQuantity').val() * +$("#modal-nBidPrice").val()).toFixed(8)} MATIC`);
                }
...
$('#BidTable').DataTable({
    ...
    aoColumns: [{
            ...
            mData: "nBidPrice",
            render: function (mData, type, row) {
                return `<td>${(mData == undefined ? '-' : mData == " " ? '-' : mData.$numberDecimal + ` MATIC`)}</td>`;
            }
        }, {
            mData: "sCreated",
            render: function (mData, type, row) {
...
function buyQuantityChange() {
    if ($('#nftQuantity').val() < 1) {
        ...
        $("#modal-final-amount").text(`${(+$('#nftQuantity').val() * +$("#modal-nBasePrice").text().split(" ")[0]).toFixed(8)} MATIC`);
